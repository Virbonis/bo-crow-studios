...
const mapDispatchToProps = dispatch => ({
  LoadMappingUser: payload => {
    dispatch({
      type: actions.LOAD_MAPPING_USER_SUB,
      payload,
      source: 'User Team',
    })
  },
  UpdateMappingUser: (payload, successCallback) => {
    dispatch({
      type: actions.UPDATE_MAPPING_USER_SUB,
      payload,
      successCallback,
      source: 'User Team',
    })
...
const User = ({
  ...
  React.useEffect(() => {
    LoadMappingUser({ team_id: data.user_team_sub_id })
...
export default connect(mapStateToProps, mapDispatchToProps)(ButtonUserSub)
