...
import { getInfoPool, poolOptionList } from './special-options'

const { Text } = Typography
const poolRow1 = [
  { title: 'SPECIALS', name: 'specials' },
  { title: 'FRAMES WINNER', name: 'frames_winner' },
]
const Pool = ({ dataHeader, dataMatchSpecial, UpdateSpecial }) => {
  const [form] = Form.useForm()

  const filteredSpecialOptions = React.useMemo(() => {
    return Object.entries(poolOptionList).reduce((acc, [title, { name, options }]) => {
      acc = {
        ...acc,
        [name]: options?.map(x => {
...
const SpecialPanel = ({ name, title, dataHeader, filteredSpecialOptions }) => {
  return (
    <>
      <Space>
        <Tooltip title={getInfoPool(title, dataHeader)}>
          <InfoCircleOutlined />
        </Tooltip>
